...
if((priceVal ^ 0) != priceVal){priceVal = priceVal.toFixed(2)}
let enterpriseVal = Number(price_wrap.querySelector('.enterprise').innerHTML)
if((enterpriseVal ^ 0) != enterpriseVal){enterpriseVal = enterpriseVal.toFixed(2)}
let sliderImgArr = []
...
shootType_triggers.forEach((tr, id)=>{
  ...
  if(!inShadow){fillForm.shadow=true;resetSelect(shadow_triggers); currentPos.shadow=null; shadow.classList.add("services-calc-cms-item-hide");errorTitle(shadowTitle, false)}else{
	  if(currentPos.shadow === null){fillForm.shadow=false;}
...
if(!currentPos.count){subtotal.innerHTML="TBD";currentPos.subtotal="TBD"; return;}
if(currentPos.count == "skip"){subtotal.innerHTML="TBD";currentPos.subtotal="TBD";return;}
...
numBtnSelect.classList.remove("button-select-wrap-hide");resultNum.classList.add("result-number-wrap-hide");formSetNum.classList.add("set-number-wrap-hide");
}
function show_2(){
errorTitle(numberTitle, false)
numBtnSelect.classList.add("button-select-wrap-hide");formSetNum.classList.remove("set-number-wrap-hide");
}
function show_3(){
errorTitle(numberTitle, false)
btnSkip.classList.remove("button-select-active");
numBtnSelect.classList.add("button-select-wrap-hide");formSetNum.classList.add("set-number-wrap-hide");resultNum.classList.remove("result-number-wrap-hide");
}
btnSpecify.addEventListener('click', ()=>{show_2(); /*console.log(currentPos)*/})// откр форм ввода кол
setNumCancel.addEventListener('click', ()=>{show_1(); /*console.log(currentPos)*/})// отм ввода кол
setNumAdd.addEventListener('click', ()=>{// добавление количества
let numImgs = Number(inputNumberImgs.value)
if(numImgs < 1){
inputNumberImgs.oninput = function() {inputNumberImgs.classList.remove("input-number-img-err")};
inputNumberImgs.classList.add("input-number-img-err");return;
}
...
addOns_list.forEach((addOns)=>{
	...
	currentPos.addons.push({name:name, count:0, price:price})
	addOns.querySelector(".add-ons-btn").addEventListener('click', ()=>{
		let modal = addOns.querySelector(".steam-modal-bg")
		let close = modal.querySelector(".cancel-icon")
		let cancel = modal.querySelector(".set-modal-cancel")
		...
		function setVal(){
			...
			onsSum.innerHTML = input.value*price
...
function smoothScroll(eID) {					 // функция прокрутки
    ...
    let vw;
   	if(width >= 991){vw = -(width/100)*5;}else{vw = -(width/100)*20;}
    stopY = stopY+vw;
	/*== кусок для моб версии ==*/
...
addToCart.addEventListener('click', ()=>{
	...
	if(!fillForm.objects){err=true;errorTitle(objectsTitle, true);scroll=true;  smoothScroll("objectsTitle") }
	if(!fillForm.shadow){err=true;errorTitle(shadowTitle, true); if(!scroll){scroll=true; smoothScroll("shadowTitle")} }
	if(!fillForm.number){err=true;errorTitle(numberTitle, true); if(!scroll){smoothScroll("numberTitle")}  }
	if(err){errBlock.classList.add("err-block-show");return}else{errBlock.classList.remove("err-block-show")}
...
firstObject.click();
	first_img.onload=function() {
		preWrap.classList.add("pre-wrap-opacity"),setTimeout(()=>{preWrap.classList.add("pre-wrap-hide")},300)
	}	
},100)
